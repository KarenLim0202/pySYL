    ...
    def dumptofile(self,outputfile):
        ...
        with open(outputfile,'w') as f:
            for k,v in d.items():
               ...
               # print(line)
                f.write(line)
    def processRun(self,outputfile):
        Process(target=self.readuser).start()
        Process(target=self.calculator).start()
        Process(target=self.dumptofile,args=(outputfile,)).start()
try:
    args=sys.argv[1:]
    index=args.index('-c')
